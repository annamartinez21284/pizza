...
class PizzaOrder(Pizza):
  #pizza = models.OneToOneField(Pizza, on_delete=models.CASCADE, related_name="item_pizza_id", parent_link=True, default=None)
  ...
  def __str__(self):
    ...
    for i in range (topping_count):
      str = "topping_"+i
      toppings.append(str)
    return f"Pizza Order: {self.order_id}, Size & Style: {self.size} {self.style}, Toppings: {self.topping_count}: {toppings}"
...
class PlatterOrder(Dish):
  ...
  def __str__(self):
    return f"Platter Order: {self.order_id}, {self.name}, size: {self.size}"

class SubOrder(Sub):
  #dish = models.ForeignKey(Dish, on_delete=models.CASCADE, related_name="dish_id_sub", parent_link=True)
  #sub_id = models.ManyToManyField(Sub, related_name="sub_id")
  ...
  def __str__(self):
    ...
    for i in range(extra_count):
      str = "extra_"+i
      extras.append(str)
    return f"Sub Order: {self.order_id}, {self.name}, size: {self.size}. {self.extra_count} Extras: {extras}"

class PastaSaladOrder(Dish):
  ...
  def __str__(self):
    return f"Pasta/Salad Order: {self.order_id}, {self.name}"
